...
const menuMaker = () => {
  ...
  const checkElements = (el, checkAgainst, isClass) => {
    try {
      if (isClass) {
        if (el.hasOwnProperty('className')) {
          return -1 === el.className.indexOf(checkAgainst);
        }
        return true;
      }
      if (el.hasOwnProperty('tagName')) {
        return checkAgainst === el.tagName.toLowerCase();
      }
      return false;
    } catch (error) {
      return false;
  ...
  const toggleFocus = (el) => {
    let self = el;
    // Move up through the ancestors of the current link until we hit .nav-menu.
    while (checkElements(self, 'nav-menu', true)) {
      // On li elements toggle the class .focus.
      if (checkElements(self, 'li', false)) {
        if (!checkElements(self, 'focus', true)) {
